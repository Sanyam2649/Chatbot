...
export function ChatInterface() {
  ...
  const handleToggleDeepThinking = () => setIsDeepThinking(!isDeepThinking);
  ...
  // 🧹 Delete chat session before starting a new one
  const handleStartNewChat = async () => {
    ...
    try {
      ...
      if (!data.success) {
        ...
        console.log("✅ Old chat session deleted:", data.message);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (messages.length >= messageLimit) return; // block input when limit reached
  ...
  return (
    <>
      {showModal ? (
        <FileUpload onClose={handleShowModal} sessionId={sessionId} />
      ) : (
                      ...
                      <div className="flex justify-end mt-1">
                        <span
                          className={`text-[10px] sm:text-xs ${
                            input.length >
                            (isDeepThinking ? 500 * 0.9 : 1000 * 0.9)
                              ? "text-red-500"
                              : "text-gray-400"
                          }`}
                        >
